...
function plane_change {
    ...
    until orbit_pos > 2 or orbit_angle < 0.2 {
        set goalorb_normal to calc_normal(goalorb).
        set ship_normal to calc_normal(ship).
        set node_vec to vcrs(goalorb_normal, ship_normal):normalized.
        set orbit_pos to vang(ship:body:position, node_vec).
    ...
    until orbit_pos > 0.5 or orbit_angle < 0.005 {
        set goalorb_normal to calc_normal(goalorb).
        set ship_normal to calc_normal(ship).
        set node_vec to vcrs(goalorb_normal, ship_normal):normalized.
        set orbit_pos to vang(ship:body:position, node_vec).
...
//define goal orbit
set goal_apo to 269297.
set goal_peri to 58477.
set goal_inc to 90.
set goal_lan to 28.4.
set goal_argpe to 210.9.
set goal_mep to 0.
set goal_t to 0.
...
lock throttle to 1.
wait until ship:orbit:apoapsis > goal_apo.
lock throttle to 0.
wait 1.
...
lock throttle to 1.
wait until ship:orbit:periapsis < goal_peri.
lock throttle to 0.
